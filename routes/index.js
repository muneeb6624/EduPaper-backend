...
router.get(
  ...
  protect,
  authorize("student"),
  attemptCtrl.startAttempt
);
router.post(
  "/papers/:paperId/submit",
  protect,
  authorize("student"),
  attemptCtrl.submitAttempt
);
router.put(
  "/attempts/:id/grade",
  protect,
  authorize("teacher"),
  attemptCtrl.gradeAttempt
);
router.get(
  "/papers/:paperId/attempts",
  protect,
  authorize("teacher"),
  attemptCtrl.getPaperAttempts
);
router.get(
  "/students/:studentId/attempts",
  protect,
  attemptCtrl.getStudentAttempts
);
...
router.post(
  ...
  protect,
  authorize("teacher"),
  resultCtrl.publishResult
);

/* -------------------------
   NOTIFICATION ROUTES
-------------------------- */
router.get("/notifications", protect, notificationCtrl.getUserNotifications);
router.put("/notifications/:id/read", protect, notificationCtrl.markAsRead);
router.put("/notifications/mark-all-read", protect, notificationCtrl.markAllAsRead);
